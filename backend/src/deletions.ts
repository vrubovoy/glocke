...
import {
  deletionJobs, inboxEvents, notifications, pushDeliveries, pushSubscriptions, users, userTombstones,
} from './db/schema.js'
...
function claims(payload: JWTPayload, audience: string) {
  const scope = payload['scope']; const jobId = payload['job_id']
  if (payload.aud !== audience || payload['token_use'] !== 'deletion' || typeof payload.sub !== 'string' ||
    !payload.sub || typeof jobId !== 'string' || !jobId || typeof payload.jti !== 'string' || !payload.jti ||
    typeof payload.exp !== 'number' || typeof scope !== 'string' || !scope.split(/\s+/).includes('account:delete')) {
...
export function createDeletionsRouter(config: { jwksUrl: string; issuer: string; service: string }) {
  const audience = `hof-deletion:${config.service}`; const jwks = createRemoteJWKSet(new URL(config.jwksUrl)); const router = new Hono()
  router.post('/account-deletions', async (c) => {
    ...
    try {
      const verified = await jwtVerify(authorization.slice(7), jwks, { algorithms: ['RS256'], issuer: config.issuer, audience, requiredClaims: ['sub', 'exp', 'jti'] })
    ...
    if (parsed.data.jobId !== principal.jobId || parsed.data.userId !== principal.userId) return c.json({ error: 'Deletion token does not match request' }, 409)
    const result = db.transaction((tx) => {
      ...
      tx.delete(pushDeliveries).where(eq(pushDeliveries.userId, principal.userId)).run()
      tx.delete(pushSubscriptions).where(eq(pushSubscriptions.userId, principal.userId)).run()
      tx.delete(notifications).where(eq(notifications.userId, principal.userId)).run()
      tx.delete(inboxEvents).where(eq(inboxEvents.userId, principal.userId)).run()
...
export const deletionsRouter = createDeletionsRouter({
  ...
  issuer: process.env['JWT_ISSUER'] ?? 'schlussel', service: 'glocke',
})
